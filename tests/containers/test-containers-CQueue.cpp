/*!
 *  \file    test-containers-CQueue.cpp Testing of the CQueue class.
 *  \brief   Testing of the CQueue class.
...
/*==========================================================================*/
/*
    FILE DESCRIPTION: Testing of the CQueue class.
  ...
  public CUnitTest
{
  ...
  // Check the fill functionality of the CQueue<Tsint>.
  void UT_ASSERT_CHECK_FILL(CQueue<Tsint>& a_rQueue)
  { CALL
    UT_ASSERT(a_rQueue.insertLast(1));
    UT_ASSERT(a_rQueue.insertLast(2));
    UT_ASSERT(a_rQueue.insertLast(3));
    UT_ASSERT(a_rQueue.insertLast(4));
    UT_ASSERT(a_rQueue.insertLast(5));
    UT_ASSERT(a_rQueue.insertLast(6));
  }

  // Check the show functionality of the CQueue<Tsint>.
  void UT_ASSERT_CHECK_SHOW(const CQueue<Tsint>& a_crQueue, const Tbool a_cDirectOrder)
  { CALL
  ...
  // Check the remove functionality of the CQueue<Tsint>.
  void UT_ASSERT_CHECK_REMOVE(CQueue<Tsint>& a_rQueue, Tbool a_cDirectOrder)
  { CALL
    Tsint value = (a_cDirectOrder) ? 1 : 6;
  ...
  { CALL
    CQueue<Tsint> instance;

    // Fill the queue.
    UT_ASSERT_CHECK_FILL(instance);

    // Show the queue before serialization.
    ...
    // Show the queue after serialization.
    UT_ASSERT_CHECK_SHOW(instance, true);
  }
} the_Test;
/*==========================================================================*/
