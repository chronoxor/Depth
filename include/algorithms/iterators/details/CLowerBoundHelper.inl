/*!
 *  \file    CLowerBoundHelper.inl Contains template implementations of the
 *           lower bound searching algorithm for different types of iterators.
 *  \brief   Lower bound searching algorithm helper class (inline).
...
/*==========================================================================*/
/*
    FILE DESCRIPTION: Lower bound searching algorithm helper class (inline).
...
*/
/*==========================================================================*/
#ifndef __CLOWERBOUNDHELPER_INL__
#define __CLOWERBOUNDHELPER_INL__
/*==========================================================================*/
...
inline T_Iterator CLowerBoundHelper<t_Algorithm>::lowerBoundNext(const T_Iterator& a_crIterator, const T_ValueType& a_crValue, T_BinaryPredicate a_fLessThan)
{ CALL
  ...
  if (it_first.isValid())
  {
    ...
    if (a_fLessThan(it_first.getValueRef(), a_crValue))
    {
      if (length == 0)
        return T_Iterator();
    }
    ...
    while (length > 0)
    {
      ...
      if (a_fLessThan(it_middle.getValueRef(), a_crValue))
      {
        it_first = it_middle;
...
template <>
template <class T_Iterator, typename T_ValueType, typename T_BinaryPredicate>
inline T_Iterator CLowerBoundHelper<1>::lowerBoundNext(const T_Iterator& a_crIterator, const T_ValueType& a_crValue, T_BinaryPredicate a_fLessThan)
{ CALL
  ...
  if (it_next.isValid())
  {
    ...
    while (it_root.isValid())
    {
      ...
      {
        it_root = it_root.getRightChildFirst();
        continue;
      }
      // Choose next search step.
      if (!a_fLessThan(it_root.getValueRef(), a_crValue))
      {
        // Update result iterator.
        it_result = it_root;
        // Next posible iterator is the left child.
        it_root = it_root.getLeftChildFirst();
      }
      else
      {
...
template <Tuint t_Algorithm>
template <class T_Iterator, typename T_ValueType, typename T_BinaryPredicate>
inline T_Iterator CLowerBoundHelper<t_Algorithm>::lowerBoundPrev(const T_Iterator& a_crIterator, const T_ValueType& a_crValue, T_BinaryPredicate a_fLessThan)
{ CALL
  ...
  if (it_first.isValid())
  {
    ...
    // Return lower bound iterator.
    if (a_fLessThan(it_result.getValueRef(), it_first.getValueRef()))
      return it_result;
    if (a_fLessThan(it_first.getValueRef(), a_crValue))
      return it_result;
    else
...
template <>
template <class T_Iterator, typename T_ValueType, typename T_BinaryPredicate>
inline T_Iterator CLowerBoundHelper<1>::lowerBoundPrev(const T_Iterator& a_crIterator, const T_ValueType& a_crValue, T_BinaryPredicate a_fLessThan)
{ CALL
  // Check T_Iterator class constraint to be an any associative iterator.
  NConcept::MConstraint::requiresConcept<NConcept::NIterators::MConceptIteratorAnyAssociative<T_Iterator> >();
  // Check T_Iterator class constraint to be an any tree iterator.
