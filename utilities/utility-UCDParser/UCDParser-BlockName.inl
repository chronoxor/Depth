/*!
 *  \file    UCDParser-BlockName.cpp Unicode characters database parser
 *           is used to parse UCD and create character properties tables for
 *           internal usage. Blocks.txt parser.
 *  \brief   Unicode characters database parser. Blocks.txt parser (source).
...
/*==========================================================================*/
/*
    FILE DESCRIPTION: Unicode characters database parser. Blocks.txt parser (source).
...
*/
/*==========================================================================*/
#ifndef __UCDPARSER_BLOCKNAME_INL__
#define __UCDPARSER_BLOCKNAME_INL__
/*==========================================================================*/
Tbool UCDParser::parseBlockName(CStreamStdOutput& a_crOutputStream)
{ CALL
  // Open Blocks.txt file.
  CFile file_Blocks(STR("Blocks.txt"));
  CHECK(file_Blocks.open(NStream::IStream::e_OPEN_READ));

  a_crOutputStream << STR("Parsing Blocks.txt file...") << ln;

  // Create the character delimiters tokenizer.
  CTokenizer tokenizer(file_Blocks.getItRead<Tschar>(), CTokenizerFunctionUCDParser());
  ...
  while (it.isValid() && !error)
  {
    ...
    if (name.insertCEnd())
    {
      CStringBuffer<Tsstr>::TIterator it_name = find(the_BlockNames, *it, FStringComparator<Tsstr, CStringASCII>());
      if (it_name.isValid())
        id = it_name.getPosition();
      else
      {
        if (!the_BlockNames.insertLast(name.isEmpty() ? ASC("") : name))
          error = true;
        else
          id = the_BlockNames.getSize() - 1;
      }
    }
    else
      error = true;
  ...
  if (error)
    a_crOutputStream << STR("Cannot parse Blocks.txt file.") << ln;
  else
    a_crOutputStream << STR("Blocks.txt file successfully parsed.") << ln;

  // Close Blocks.txt file.
  CHECK(file_Blocks.close());
