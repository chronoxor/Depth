/*!
 *  \file    example-geometry-CVector2D.cpp Example of using the CVector2D
 *           class.
 *  \brief   Example of using the CVector2D class.
...
/*==========================================================================*/
/*
    FILE DESCRIPTION: Example of using the CVector2D class.
  ...
  public NApplication::IApplication
{
  ...
  { CALL
    ...
    CVector2D vector;
    stdoutput << STR("CVector2D vector;") << ln << STR(" ---> ") << vector << ln;

    vector.set(10.0, 0.0);
    stdoutput << STR("vector.set(10.0, 0.0);") << ln << STR(" ---> ") << vector << ln;

    vector.normalize();
    stdoutput << STR("vector.normalize();") << ln << STR(" ---> ") << vector << ln;

    vector = vector.translate(4.0, 5.0);
    stdoutput << STR("vector = vector.translate(4.0, 5.0);") << ln << STR(" ---> ") << vector << ln;

    vector = vector.scale(CONSTR(0.2), CONSTR(0.2));
    stdoutput << STR("= vector.scale(CONSTR(0.2), CONSTR(0.2));") << ln << STR(" ---> ") << vector << ln;

    vector = vector.rotate(CMathConstants::PI_2);
    stdoutput << STR("vector = vector.rotate(CMathConstants::PI_2);") << ln << STR(" ---> ") << vector << ln;
