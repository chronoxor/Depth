/*!
 *  \file    CFilterXXE.cpp XXE string filter class converts given ASCII
 *           and Unicode strings into the corresponding XXE-encoded
 *           representation.
 *  \brief   XXE string filter class (source).
...
/*==========================================================================*/
/*
    FILE DESCRIPTION: XXE string filter class (source).
...
/*==========================================================================*/
#ifndef __CFILTERXXE_CPP__
#define __CFILTERXXE_CPP__
/*==========================================================================*/
...
/*==========================================================================*/
BASE_API Tbool CFilterXXE::m_sIsInitialized = false;
BASE_API Tbyte CFilterXXE::m_sEncodeTable[64] =
{
  Tbyte('+'),  Tbyte('-'),  Tbyte('0'),  Tbyte('1'),
  Tbyte('2'),  Tbyte('3'),  Tbyte('4'),  Tbyte('5'),
  Tbyte('6'),  Tbyte('7'),  Tbyte('8'),  Tbyte('9'),
...
BASE_API Tbyte CFilterXXE::m_sDecodeTable[256];
/*--------------------------------------------------------------------------*/
Tbool CFilterXXE::set(const CFilterXXE& a_crInstance)
{ CALL
  // Check if the given class instance is not the same to the current one.
  ASSERT((&a_crInstance != this), STR("Setting the same CFilterXXE class instance."))
  {
    return false;
  }
  ...
  // Initialize XXE filter properties.
  m_IsWriteLineEnding = a_crInstance.m_IsWriteLineEnding;
  return true;
}
/*--------------------------------------------------------------------------*/
Tbool CFilterXXE::onOpenReadFrame(IReader& a_rReader, Tbuffer a_pBuffer, Tuint& a_rCurrentSize, const Tuint a_cFullSize)
{ CALL
...
/*--------------------------------------------------------------------------*/
Tbool CFilterXXE::onOpenWriteFrame(IWriter& a_rWriter, Tbuffer a_pBuffer, Tuint& a_rCurrentSize, const Tuint a_cFullSize)
{ CALL
...
/*--------------------------------------------------------------------------*/
Tbool CFilterXXE::onPackReadFrame(IReader& a_rReader, Tbuffer a_pBuffer, Tuint& a_rCurrentSize, const Tuint a_cFullSize)
{ CALL
  Tuint line = 0;
  ...
  // Check if the length of XXE line is less or equal to 45.
  ASSERT(((line > 0) && (line <= 45)), STR("XXE encoded stream has incorrect format (length of XXE line = %u).") COMMA line)
  {
    return false;
  }
  // Check if the length read buffer is enough to unpack XXE line.
  ASSERT((line <= count), STR("Length of the read buffer of the XXE filter is not enough to unpack XXE line (length of XXE line = %u, XXE read buffer length = %u).") COMMA line COMMA count)
  {
    return false;
  }
  ...
  while (line > 0)
  {
    // Read XXE encoded characters form the given reader.
    ...
    {
      WARNING(STR("XXE encoded stream has incorrect format."));
      return false;
    }

    // Unpack XXE encoded characters.
    ...
    {
      WARNING(STR("XXE encoded binary stream contains invalid characters (%hU, %hU and %hU).") COMMA (Tuint)b1 COMMA (Tuint)b2 COMMA (Tuint)b3);
      return false;
    }
...
/*--------------------------------------------------------------------------*/
Tbool CFilterXXE::onPackWriteFrame(IWriter& a_rWriter, Tbuffer a_pBuffer, Tuint& a_rCurrentSize, const Tuint a_cFullSize)
{ CALL
  IGNORE_UNUSED(a_cFullSize);
  ...
  {
    WARNING(STR("Cannot write XXE counter character into the stream."));
    return false;
  }
...
/*--------------------------------------------------------------------------*/
Tbool CFilterXXE::onCloseReadFrame(IReader& a_rReader, Tbuffer a_pBuffer, Tuint& a_rCurrentSize, const Tuint a_cFullSize)
{ CALL
  IGNORE_UNUSED(a_rReader);
  IGNORE_UNUSED(a_pBuffer);
  IGNORE_UNUSED(a_rCurrentSize);
...
/*--------------------------------------------------------------------------*/
Tbool CFilterXXE::onCloseWriteFrame(IWriter& a_rWriter, Tbuffer a_pBuffer, Tuint& a_rCurrentSize, const Tuint a_cFullSize)
{ CALL
  return onPackWriteFrame(a_rWriter, a_pBuffer, a_rCurrentSize, a_cFullSize);
}
/*--------------------------------------------------------------------------*/
void CFilterXXE::calculateTables()
{ CALL
  if (!m_sIsInitialized)
  {
